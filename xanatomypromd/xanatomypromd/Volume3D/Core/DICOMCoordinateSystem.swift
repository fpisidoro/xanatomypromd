...
class DICOMCoordinateSystem: ObservableObject {
    ...
    func worldToScreen(
        ...
        // Get plane dimensions and spacing
        let planeDims = plane.volumeDimensions(from: volumeDimensions)
        let planeSpacing = plane.volumeSpacing(from: volumeSpacing)
        
        // Convert world position to voxel coordinates for this plane
        let worldRelativeToOrigin = position - volumeOrigin
        
        let voxelCoords: SIMD2<Float>
        switch plane {
        case .axial:
            voxelCoords = SIMD2<Float>(
                worldRelativeToOrigin.x / volumeSpacing.x,
                worldRelativeToOrigin.y / volumeSpacing.y
            )
        case .sagittal:
            voxelCoords = SIMD2<Float>(
                worldRelativeToOrigin.y / volumeSpacing.y,
                worldRelativeToOrigin.z / volumeSpacing.z
            )
        case .coronal:
            voxelCoords = SIMD2<Float>(
                worldRelativeToOrigin.x / volumeSpacing.x,
        ...
        let normalizedCoords = SIMD2<Float>(
            voxelCoords.x / Float(planeDims.x),
            voxelCoords.y / Float(planeDims.y)
        )
        
        // Convert to screen coordinates within image bounds
        let screenX = bounds.minX + CGFloat(normalizedCoords.x) * bounds.width
        let screenY = bounds.minY + CGFloat(normalizedCoords.y) * bounds.height
    ...
    /// Convert 2D screen coordinates to 3D world coordinates (mm)
    func screenToWorld(
        screenPoint: CGPoint,
        plane: MPRPlane,
        viewSize: CGSize,
        ...
        let voxelX = normalizedX * Float(planeDims.x)
        let voxelY = normalizedY * Float(planeDims.y)
        
        // Convert to world coordinates and combine with current position
        var worldPosition = currentWorldPosition
        
        switch plane {
        case .axial:
            worldPosition.x = volumeOrigin.x + voxelX * volumeSpacing.x
            worldPosition.y = volumeOrigin.y + voxelY * volumeSpacing.y
            // Z stays the same (current slice)
            
        case .sagittal:
            worldPosition.y = volumeOrigin.y + voxelX * volumeSpacing.y
            worldPosition.z = volumeOrigin.z + voxelY * volumeSpacing.z
            // X stays the same (current slice)
            
        case .coronal:
            worldPosition.x = volumeOrigin.x + voxelX * volumeSpacing.x
            worldPosition.z = volumeOrigin.z + voxelY * volumeSpacing.z
            // Y stays the same (current slice)
        }
